...
class KafkaDeserializationDltTest {
    ...
    void routesOriginalBytesFromDeserializationFailureToDlt() {
        byte[] malformedAvro = new byte[] {61, 62, 63, 64};
        RecordHeaders headers = new RecordHeaders();

        ErrorHandlingDeserializer<byte[]> deserializer =
                new ErrorHandlingDeserializer<>(new AlwaysFailingDeserializer());

        byte[] value = deserializer.deserialize("file.ready.match-summary", headers, malformedAvro);
        ...
        DeadLetterPublishingRecoverer recoverer =
                new KafkaErrorHandlingConfig().deadLetterPublishingRecoverer(
                        template, "file.ready.match-summary.DLT");

        ConsumerRecord<Object, Object> failedRecord =
                new ConsumerRecord<>("file.ready.match-summary", 4, 21L, null, value);
        ...
        assertThat(dltRecord.topic()).isEqualTo("file.ready.match-summary.DLT");
        assertThat(dltRecord.partition()).isNull();
        assertThat(dltRecord.value()).isEqualTo(malformedAvro);
    }

    @Test
    void createsDelegatingSerializerForAvroAndRawBytes() {
        assertThat(new KafkaErrorHandlingConfig().delegatingAvroSerializer()).isNotNull();
    }
